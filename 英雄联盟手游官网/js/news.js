(function () {
    ...
    // 新闻数据
    ajax("https://apps.game.qq.com/cmc/cross?serviceId=166&source=web_pc&filter=channel&chanid=4897&typeids=1&limit=4&start=0&sortby=sIdxTime").then(res => {
        let data = res.data.items;
        let listHtml = '';
        data.forEach(ele => {
            let type = ele.sTagInfo.split(",")[0].split("|")[1];
            listHtml += `
            <li>
                <span>${type}</span>
                <p>${ele.sIdxTime}</p>
                <a href="https://lolm.qq.com/m/news_detail.html?docid=${ele.iDocID}" target="_blank">
                    ${ele.sTitle}
                </a>
            </li> 
            `
        })
        $(".news-container .news-list").innerHTML = listHtml;
